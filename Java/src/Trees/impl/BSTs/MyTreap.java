 ...
 */
public class MyTreap<T extends Comparable<T>> extends MyBST<T> {
    ...
    public MyTreap(MyTreap<T> treapIn) {
        root = null;
        size = 0;
        priorities = new MyCAHashSet<>();
    ...
    /**
     * Inserts each element in {@code keysIn} with the same-index priority in {@code prioritiesIn},
     * starting at index 0.
     * 
     * Returns {@code true} if for each element in {@code keysIn}, inserting the element with the
     * same-index priority in {@code priorities} changes the structure of the tree. Returns
     * {@code false} otherwise.
     * 
     * @throws UnsupportedOperationException if {@code keysIn} and {@code prioritiesIn} are of
     *         unequal length.
     */
    ...
    public boolean insert(Object[] keysIn) {
        for (int index = 0; index < keysIn.length; index++) {
            ...
            boolean inserted = insert(keysIn[index]);
            if (!inserted) {
                return false;
            }
        }
    ...
    public boolean insert(Object keyIn, int priorityIn) {
        ...
        if (priorities.contains(priorityIn)) {
            throw new UnsupportedOperationException(
                    "A node with priority " + priorityIn + " already exists!");
        }
        ...
        if (!root.key.getClass().isAssignableFrom(keyIn.getClass())) {
            throw new ClassCastException(keyIn.getClass().getName()
                    + " is not compatible with a tree of type " + root.key.getClass().getName());
        }
        priorities.add(priorityIn);
        ...
        // handle rotations while the added node isn't the root and has a priority greater than its
        // parent's
    ...
    public boolean delete(Object o) {
        ...
        while (!isLeaf(toDelete)) {
            // System.out.println(this + "\n-----------------------------------\n");
            // System.out.println("prior to rotation down, 'toDelete.parent = " +
            // (toDelete.parent));

            // must perform left rotation on right child since there's no left child
            if (toDelete.left == null && toDelete.right != null) {
                // System.out.println("performing left rotation on right child since left child =
                // null\n");
                rotate(toDelete.right, true);
            }
            // must perform right rotation on left child since there's no right child
            else if (toDelete.left != null && toDelete.right == null) {
                // System.out.println("performing right rotation on left child since right child =
                // null\n");
                rotate(toDelete.left, false);
            }
            // perform appropriate rotation based on which child has higher priority
            else {
                // System.out.println("performing appropriate rotation on child since both children
                // exist\n");
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof MyTreap)) {
            return false;
        }
        MyTreap<T> other = (MyTreap<T>) o;
    ...
    /**
     * Returns a List containing this Tree's elements following a preorder traversal.
     */
    @Override
    ...
    /**
     * Returns a List containing this Tree's elements following a postorder traversal.
     */
    @Override
    ...
    public boolean retainAll(Collection<?> c) {
        ...
        int initialSize = 0;
        MyTreap<T> toRemove = new MyTreap<>();
        for (T element : this) {
    ...
    // for some reason a constructor given a pre-existing MyTreapSet wouldn't work, so this is a
    // substitute
    public MyTreap<T> deepClone() {
        MyTreap<T> clone = new MyTreap<T>();
